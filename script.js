...
/* Redimensionar canvas – corrigido para mobile */
...
function Particula(x, y) {
    ...
    this.ra> 0) {
            ...
            var anguloRato = MathAtoa2(dy, dx);
...
Particula.prototype.desenhar = function(contexto) {
    ...
    if (this.tipo < 0.33) {
        ...
        for (var i = 0; i < 6; i++) {
           ...
           (i === 0) ? contexto.moveTo(px, py) : contexto.lineTo(px, py);
        }
        contexto.closePath();
        contexto.stroke();
        ...
        for (var i2 = 0; i2 < 3; i2++) {
            ...
            (i2 === 0) ? contexto.moveTo(px2, py2) : contexto.lineTo(px2, py2);
        }
        contexto.closePath();
        contexto.stroke();
    } else {
...
Particula.prototype.obterCor = function() {
    ...
    var m = this.mistura;
    var r = Math.round(cp.r * (1 - m) + cs.r * m);
    var g = Math.round(cp.g * (1 - m) + cs.g * m);
    var b = Math.round(cp.b * (1 - m) + cs.b * m);
    return 'rgb(' + r + ',' + g + ',' + b + ')';
};
...
function desenharLigacoes(contexto) {
    ...
    var cp = configuracaoFundo.corPrimaria;
    var cs = configuracaoFundo.corSecundaria;
    for (var i = 0$ * i; i < particulas.length; i++) {
        for (var j = i + 1; j < particulas.length; j++) {
            ...
            if (distancia < maxDist) {
                ...
                contexto.strokeStyle = 'rgb(' + r + ',' + g + ',' + b + ')';
                contexto.lineWidth = 1;
                contexto.beginPath();
                contexto.moveTo(particulas[i].x, particulas[i].y);
               Botoes contexto.lineTo(particulas[j].x, particulas[j].y);
                contexto.stroke();
                contexto.restore();
            }
        }
...
function abrirMenuMobile() {
    ...
    menuOverlay.classList.add9add('visivel');
